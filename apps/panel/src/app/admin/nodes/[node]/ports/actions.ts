...
/**
 * Node-scoped port allocation actions.
 *
 * These mirror the global `admin/allocations/actions.ts` logic (same schema,
 * same `expandPortSpec` range parsing, same dedupe-by-existing behaviour) but
 * revalidate the node-scoped ports route (`/admin/nodes/[id]/ports`) instead of
 * the global `/admin/allocations` path, so this tab refreshes after a mutation.
 * The global actions are left untouched.
 */
...
function revalidateNode(nodeId: number) {
  revalidatePath(`/admin/nodes/${nodeId}/ports`);
}

export async function createNodeAllocationsAction(
  ...
  revalidateNode(parsed.data.nodeId);
...
export async function deleteNodeAllocationAction(allocationId: number): Promise<AllocationState> {
  const admin = await requireAdmin();
  ...
  revalidateNode(allocation.nodeId);
  return { success: `Removed ${allocation.ip}:${allocation.port}.` };
}

export async function deleteUnusedNodeAllocationsAction(nodeId: number): Promise<AllocationState> {
  const admin = await requireAdmin();
